extends RigidBody

const MOVE_SPEED = 4.8
const MOVE_ACCEL = 6.8
const AIR_CONTROL = 0.34
const MAX_FORCE = 520.0
const JUMP_IMPULSE = 6.2
const UPRIGHT_STRENGTH = 34.0
const UPRIGHT_DAMPING = 7.0
const TURN_STRENGTH = 7.0

var move_input = Vector2.ZERO
var control_yaw = 0.0
var jump_requested = false
var grounded = false
var walk_time = 0.0
var limbs = []
var torso = null
var head = null

func _ready():
    mass = 2.8
    linear_damp = 0.55
    angular_damp = 0.85
    contact_monitor = true
    contacts_reported = 12
    continuous_cd = true
    can_sleep = false
    _build_collision()
    _build_character()

func _build_collision():
    var collider = CollisionShape.new()
    var capsule = CapsuleShape.new()
    capsule.radius = 0.42
    capsule.height = 2.25
    collider.shape = capsule
    collider.translation.y = -0.05
    add_child(collider)

func _mat(color, roughness = 0.78):
    var material = SpatialMaterial.new()
    material.albedo_color = color
    material.roughness = roughness
    return material

func _mesh_part(name_text, mesh, position, rotation, material):
    var node = MeshInstance.new()
    node.name = name_text
    node.mesh = mesh
    node.translation = position
    node.rotation = rotation
    node.material_override = material
    node.cast_shadow = GeometryInstance.SHADOW_CASTING_SETTING_ON
    add_child(node)
    return node

func _build_character():
    var suit = _mat(Color("f0f2f4"))
    var accent = _mat(Color("6aa5d8"), 0.65)
    var dark = _mat(Color("18202a"), 0.9)

    var torso_mesh = CapsuleMesh.new()
    torso_mesh.radius = 0.39
    torso_mesh.mid_height = 0.40
    torso = _mesh_part("Torso", torso_mesh, Vector3(0, 0.05, 0), Vector3.ZERO, suit)

    var belly = CubeMesh.new()
    belly.size = Vector3(0.56, 0.34, 0.06)
    _mesh_part("Accent", belly, Vector3(0, 0.06, -0.37), Vector3.ZERO, accent)

    var head_mesh = SphereMesh.new()
    head_mesh.radius = 0.38
    head_mesh.height = 0.76
    head = _mesh_part("Head", head_mesh, Vector3(0, 0.94, -0.02), Vector3.ZERO, suit)

    var eye_mesh = SphereMesh.new()
    eye_mesh.radius = 0.042
    eye_mesh.height = 0.084
    _mesh_part("EyeL", eye_mesh, Vector3(-0.12, 1.00, -0.355), Vector3.ZERO, dark)
    _mesh_part("EyeR", eye_mesh, Vector3(0.12, 1.00, -0.355), Vector3.ZERO, dark)

    var arm_mesh = CapsuleMesh.new()
    arm_mesh.radius = 0.13
    arm_mesh.mid_height = 0.66
    var left_arm = _mesh_part("LeftArm", arm_mesh, Vector3(-0.61, 0.22, 0), Vector3(0, 0, deg2rad(76)), suit)
    var right_arm = _mesh_part("RightArm", arm_mesh, Vector3(0.61, 0.22, 0), Vector3(0, 0, deg2rad(-76)), suit)
    limbs.append(left_arm)
    limbs.append(right_arm)

    var leg_mesh = CapsuleMesh.new()
    leg_mesh.radius = 0.16
    leg_mesh.mid_height = 0.60
    var left_leg = _mesh_part("LeftLeg", leg_mesh, Vector3(-0.22, -0.78, 0.02), Vector3.ZERO, suit)
    var right_leg = _mesh_part("RightLeg", leg_mesh, Vector3(0.22, -0.78, 0.02), Vector3.ZERO, suit)
    limbs.append(left_leg)
    limbs.append(right_leg)

    var foot_mesh = CubeMesh.new()
    foot_mesh.size = Vector3(0.30, 0.18, 0.50)
    _mesh_part("LeftFoot", foot_mesh, Vector3(-0.22, -1.18, -0.11), Vector3.ZERO, suit)
    _mesh_part("RightFoot", foot_mesh, Vector3(0.22, -1.18, -0.11), Vector3.ZERO, suit)

func set_controls(move, jump, yaw):
    move_input = move
    if move_input.length() > 1.0:
        move_input = move_input.normalized()
    control_yaw = yaw
    if jump:
        jump_requested = true

func _physics_process(delta):
    _update_grounded()
    _animate_soft_body(delta)
    if global_transform.origin.y < -15.0:
        linear_velocity = Vector3.ZERO
        angular_velocity = Vector3.ZERO
        var t = global_transform
        t.origin = Vector3(0, 3.0, 0)
        t.basis = Basis()
        global_transform = t

func _update_grounded():
    var world = get_world()
    if world == null:
        grounded = false
        return
    var from = global_transform.origin + Vector3.UP * 0.05
    var to = from + Vector3.DOWN * 1.38
    var hit = world.direct_space_state.intersect_ray(from, to, [self], 1)
    grounded = not hit.empty()

func _integrate_forces(state):
    var forward = Vector3(-sin(control_yaw), 0.0, -cos(control_yaw))
    var right = Vector3(cos(control_yaw), 0.0, -sin(control_yaw))
    var wish_dir = right * move_input.x + forward * move_input.y
    if wish_dir.length_squared() > 0.001:
        wish_dir = wish_dir.normalized()

    var lv = state.linear_velocity
    var horizontal = Vector3(lv.x, 0.0, lv.z)
    var target = wish_dir * MOVE_SPEED
    var control = 1.0 if grounded else AIR_CONTROL
    var force = (target - horizontal) * mass * MOVE_ACCEL * control
    if force.length() > MAX_FORCE:
        force = force.normalized() * MAX_FORCE
    state.add_central_force(force)

    var up = state.transform.basis.y.normalized()
    var tilt_axis = up.cross(Vector3.UP)
    var tilt_dot = clamp(up.dot(Vector3.UP), -1.0, 1.0)
    var tilt_angle = acos(tilt_dot)
    if tilt_axis.length_squared() > 0.00001:
        var balance = tilt_axis.normalized() * tilt_angle * UPRIGHT_STRENGTH
        var damp = Vector3(state.angular_velocity.x, 0.0, state.angular_velocity.z) * UPRIGHT_DAMPING
        state.add_torque(balance - damp)

    if wish_dir.length_squared() > 0.01:
        var body_forward = -state.transform.basis.z
        body_forward.y = 0.0
        if body_forward.length_squared() > 0.001:
            body_forward = body_forward.normalized()
            var signed_turn = body_forward.cross(wish_dir).y
            state.add_torque(Vector3.UP * signed_turn * TURN_STRENGTH)

    if jump_requested:
        if grounded:
            state.apply_central_impulse(Vector3.UP * JUMP_IMPULSE)
            state.apply_torque_impulse(Vector3(move_input.y, 0.0, -move_input.x) * 0.75)
        jump_requested = false

    if state.linear_velocity.length() > 17.0:
        state.linear_velocity = state.linear_velocity.normalized() * 17.0
    if state.angular_velocity.length() > 11.0:
        state.angular_velocity = state.angular_velocity.normalized() * 11.0

func _animate_soft_body(delta):
    var speed = Vector2(linear_velocity.x, linear_velocity.z).length()
    walk_time += delta * (2.0 + speed * 1.8)
    var amount = clamp(speed / MOVE_SPEED, 0.0, 1.0)
    var flop = sin(walk_time) * 0.34 * amount
    if limbs.size() >= 4:
        limbs[0].rotation.z = deg2rad(76) + flop * 0.55
        limbs[1].rotation.z = deg2rad(-76) - flop * 0.55
        limbs[2].rotation.x = flop
        limbs[3].rotation.x = -flop
    if head != null:
        head.rotation.z = sin(walk_time * 0.55) * 0.055 * amount
    if torso != null:
        torso.rotation.z = sin(walk_time * 0.5) * 0.035 * amount
