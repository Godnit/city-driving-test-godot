...
func _build_ground() -> void:
	...
	add_child(ground)
	var floor_mesh_node := MeshInstance3D.new()
	...
	var light_line_mat := StandardMaterial3D.new()
	light_line_mat.albedo_color = Color("aeb5bb")
	light_line_mat.roughness = 1.0
	var dark_line_mat := StandardMaterial3D.new()
...
func _update_camera(delta: float) -> void:
	var target := player.global_position + Vector3.UP * 0.45
