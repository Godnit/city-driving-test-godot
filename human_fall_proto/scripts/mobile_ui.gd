...
func _rescale() -> void:
	...
	var scale_factor: float = clampf(minf(s.x / 960.0, s.y / 540.0), 0.78, 1.55)
...
func _handle_drag(event: InputEventScreenDrag) -> void:
	if event.index == _move_finger:
		var drag_delta: Vector2 = event.position - _joystick_center
		var limited: Vector2 = drag_delta.limit_length(_joystick_radius)
...
func consume_jump() -> bool:
	var value: bool = _jump_buffered
	_jump_buffered = false
	return value

func consume_camera_delta() -> Vector2:
	var value: Vector2 = _camera_delta
...
func _draw() -> void:
	...
	var jump_alpha: float = 0.58 if _jump_finger != -1 else 0.34
	...
	var r: float = _jump_radius * 0.34
	var tri: PackedVector2Array = PackedVector2Array([_jump_center + Vector2(0, -r), _jump_center + Vector2(r * 0.78, r * 0.45), _jump_center + Vector2(-r * 0.78, r * 0.45)])
