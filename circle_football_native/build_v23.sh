...
# Build from our stable role/team gameplay, then v2.2 equal-speed base, then v2.3 fixes.
python3 v17_run.py
python3 v18_run.py
python3 v22_patch.py
python3 v23_patch.py

grep -q 'float speed=185f\*s;' src/com/godnit/circlefootballlite/MainActivity.java
grep -q 'void resolveGoalPost' src/com/godnit/circlefootballlite/MainActivity.java
grep -q 'float kickReach(){return discR+ballR+24f\*s;}' src/com/godnit/circlefootballlite/MainActivity.java
grep -q 'versionName="2.3.0"' AndroidManifest.xml
...
# Trim leading silence and long tails from short events. This keeps each sound tied
# tightly to the exact kick / bounce / goal event and removes the delayed feeling.
ffmpeg -hide_banner -loglevel error -y -i "$A/kick_a.mp3" -af "silenceremove=start_periods=1:start_duration=0.01:start_threshold=-45dB,volume=0.92" -t 0.62 -ac 1 -ar 22050 -b:a 80k res/raw/kick_a.mp3
ffmpeg -hide_banner -loglevel error -y -i "$A/kick_b.mp3" -af "silenceremove=start_periods=1:start_duration=0.01:start_threshold=-45dB,volume=0.90" -t 0.62 -ac 1 -ar 22050 -b:a 80k res/raw/kick_b.mp3
ffmpeg -hide_banner -loglevel error -y -i "$A/ball_bounce.mp3" -af "silenceremove=start_periods=1:start_duration=0.01:start_threshold=-46dB,volume=0.82" -t 0.48 -ac 1 -ar 22050 -b:a 72k res/raw/ball_bounce.mp3
ffmpeg -hide_banner -loglevel error -y -i "$A/goal_cheer.mp3" -af "silenceremove=start_periods=1:start_duration=0.01:start_threshold=-46dB,volume=0.96" -t 3.2 -ac 1 -ar 22050 -b:a 80k res/raw/goal_cheer.mp3
ffmpeg -hide_banner -loglevel error -y -i "$A/crowd_burst.mp3" -af "silenceremove=start_periods=1:start_duration=0.01:start_threshold=-46dB,volume=0.82" -t 2.1 -ac 1 -ar 22050 -b:a 72k res/raw/crowd_burst.mp3
ffmpeg -hide_banner -loglevel error -y -i "$A/ui_click.mp3" -af "silenceremove=start_periods=1:start_duration=0.01:start_threshold=-48dB,volume=0.70" -t 0.34 -ac 1 -ar 22050 -b:a 64k res/raw/ui_click.mp3
...
for f in kick_a kick_b ball_bounce goal_cheer crowd_burst ui_click crowd_loop menu_music; do test -s "res/raw/$f.mp3"; done
...
"$BT/apksigner" sign --ks build/app.keystore --ks-pass pass:android --key-pass pass:android --out build/CircleFootball-Android81-v2.3.apk build/aligned.apk
"$BT/apksigner" verify --verbose build/CircleFootball-Android81-v2.3.apk
"$BT/aapt" dump badging build/CircleFootball-Android81-v2.3.apk | tee build/badging.txt
grep -q "package: name='com.godnit.circlefootballlite.v23'" build/badging.txt
grep -q "versionCode='14'" build/badging.txt
grep -q "versionName='2.3.0'" build/badging.txt
grep -q "sdkVersion:'21'" build/badging.txt
unzip -l build/CircleFootball-Android81-v2.3.apk | tee build/apk-files.txt
grep -q 'res/raw/kick_a.mp3' build/apk-files.txt
grep -q 'res/raw/crowd_loop.mp3' build/apk-files.txt
echo "APK_SIZE_BYTES=$(stat -c%s build/CircleFootball-Android81-v2.3.apk)" | tee build/size.txt
