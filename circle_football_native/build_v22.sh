...
# Stable custom arcade gameplay + real audio, then only slow/equalize movement.
python3 v17_run.py
python3 v18_run.py
python3 v22_patch.py

grep -q 'float speed=220f\*s;' src/com/godnit/circlefootballlite/MainActivity.java
grep -q 'versionName="2.2.0"' AndroidManifest.xml
...
"$BT/apksigner" sign --ks build/app.keystore --ks-pass pass:android --key-pass pass:android --out build/CircleFootball-Android81-v2.2.apk build/aligned.apk
"$BT/apksigner" verify --verbose build/CircleFootball-Android81-v2.2.apk
"$BT/aapt" dump badging build/CircleFootball-Android81-v2.2.apk | tee build/badging.txt
grep -q "package: name='com.godnit.circlefootballlite.v22'" build/badging.txt
grep -q "versionCode='13'" build/badging.txt
grep -q "versionName='2.2.0'" build/badging.txt
grep -q "sdkVersion:'21'" build/badging.txt
unzip -l build/CircleFootball-Android81-v2.2.apk | tee build/apk-files.txt
grep -q 'res/raw/kick_a.mp3' build/apk-files.txt
grep -q 'res/raw/crowd_loop.mp3' build/apk-files.txt
echo "APK_SIZE_BYTES=$(stat -c%s build/CircleFootball-Android81-v2.2.apk)" | tee build/size.txt
