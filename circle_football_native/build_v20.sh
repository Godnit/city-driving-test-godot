...
python3 v17_run.py
python3 v18_run.py
python3 v19_patch.py
python3 v20_patch.py

grep -q 'boolean humanHasBall=false' src/com/godnit/circlefootballlite/MainActivity.java
grep -q 'void doKickAim' src/com/godnit/circlefootballlite/MainActivity.java
grep -q 'void updateKickAim' src/com/godnit/circlefootballlite/MainActivity.java
grep -q 'versionName="2.0.0"' AndroidManifest.xml
...
"$BT/apksigner" sign --ks build/app.keystore --ks-pass pass:android --key-pass pass:android --out build/CircleFootball-Android81-v2.0.apk build/aligned.apk
"$BT/apksigner" verify --verbose build/CircleFootball-Android81-v2.0.apk
"$BT/aapt" dump badging build/CircleFootball-Android81-v2.0.apk | tee build/badging.txt
grep -q "package: name='com.godnit.circlefootballlite.v20'" build/badging.txt
grep -q "versionCode='11'" build/badging.txt
grep -q "versionName='2.0.0'" build/badging.txt
grep -q "sdkVersion:'21'" build/badging.txt
unzip -l build/CircleFootball-Android81-v2.0.apk > build/apk-files.txt
echo "APK_SIZE_BYTES=$(stat -c%s build/CircleFootball-Android81-v2.0.apk)" | tee build/size.txt
