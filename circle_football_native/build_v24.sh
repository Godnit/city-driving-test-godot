...
# Stable custom arcade gameplay + real audio + slower v2.3 base, then v2.4 visual/physics fixes.
python3 v17_run.py
python3 v18_run.py
...
grep -q 'fieldTheme=0' src/com/godnit/circlefootballlite/MainActivity.java
grep -q 'drawPredictionGuideV24' src/com/godnit/circlefootballlite/MainActivity.java
grep -q 'stabilizeBallSqueezeV24' src/com/godnit/circlefootballlite/MainActivity.java
grep -q 'wholeBetweenPosts' src/com/godnit/circlefootballlite/MainActivity.java
grep -q 'versionName="2.4.0"' AndroidManifest.xml
...
"$BT/apksigner" sign --ks build/app.keystore --ks-pass pass:android --key-pass pass:android --out build/CircleFootball-Android81-v2.4.apk build/aligned.apk
"$BT/apksigner" verify --verbose build/CircleFootball-Android81-v2.4.apk
"$BT/aapt" dump badging build/CircleFootball-Android81-v2.4.apk | tee build/badging.txt
grep -q "package: name='com.godnit.circlefootballlite.v24'" build/badging.txt
grep -q "versionCode='15'" build/badging.txt
grep -q "versionName='2.4.0'" build/badging.txt
grep -q "sdkVersion:'21'" build/badging.txt
unzip -l build/CircleFootball-Android81-v2.4.apk | tee build/apk-files.txt
grep -q 'res/raw/kick_a.mp3' build/apk-files.txt
grep -q 'res/raw/crowd_loop.mp3' build/apk-files.txt
echo "APK_SIZE_BYTES=$(stat -c%s build/CircleFootball-Android81-v2.4.apk)" | tee build/size.txt
