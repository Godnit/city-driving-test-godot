...
# Rebuild cleanly from the stable team/AI base + real-audio release, then add
# the HaxBall-style universal physics. v1.9/v2.0 possession patches are intentionally skipped.
python3 v17_run.py
python3 v18_run.py
python3 v21_patch.py

grep -q 'Same rule as the human' src/com/godnit/circlefootballlite/MainActivity.java
grep -q 'float kickReach(){return discR+ballR+12f\*s;}' src/com/godnit/circlefootballlite/MainActivity.java
grep -q 'versionName="2.1.0"' AndroidManifest.xml
...
"$BT/apksigner" sign --ks build/app.keystore --ks-pass pass:android --key-pass pass:android --out build/CircleFootball-Android81-v2.1.apk build/aligned.apk
"$BT/apksigner" verify --verbose build/CircleFootball-Android81-v2.1.apk
"$BT/aapt" dump badging build/CircleFootball-Android81-v2.1.apk | tee build/badging.txt
grep -q "package: name='com.godnit.circlefootballlite.v21'" build/badging.txt
grep -q "versionCode='12'" build/badging.txt
grep -q "versionName='2.1.0'" build/badging.txt
grep -q "sdkVersion:'21'" build/badging.txt
unzip -l build/CircleFootball-Android81-v2.1.apk | tee build/apk-files.txt
grep -q 'res/raw/kick_a.mp3' build/apk-files.txt
grep -q 'res/raw/crowd_loop.mp3' build/apk-files.txt
echo "APK_SIZE_BYTES=$(stat -c%s build/CircleFootball-Android81-v2.1.apk)" | tee build/size.txt
