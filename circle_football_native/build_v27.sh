...
grep -q 'countdownTimer' src/com/godnit/circlefootballlite/MainActivity.java
grep -q 'escapePassV27' src/com/godnit/circlefootballlite/MainActivity.java
grep -q 'drawCountdownV27' src/com/godnit/circlefootballlite/MainActivity.java
grep -q 'splashUntil=SystemClock.uptimeMillis()+2600L' src/com/godnit/circlefootballlite/MainActivity.java
grep -q 'versionName="2.7.0"' AndroidManifest.xml
...
"$BT/apksigner" sign --ks build/app.keystore --ks-pass pass:android --key-pass pass:android --out build/CircleFootball-Android81-v2.7.apk build/aligned.apk
"$BT/apksigner" verify --verbose build/CircleFootball-Android81-v2.7.apk
"$BT/aapt" dump badging build/CircleFootball-Android81-v2.7.apk | tee build/badging.txt
grep -q "package: name='com.godnit.circlefootballlite.v27'" build/badging.txt
grep -q "versionCode='18'" build/badging.txt
grep -q "versionName='2.7.0'" build/badging.txt
grep -q "sdkVersion:'21'" build/badging.txt
unzip -l build/CircleFootball-Android81-v2.7.apk | tee build/apk-files.txt
grep -q 'res/raw/kick_a.mp3' build/apk-files.txt
grep -q 'res/raw/crowd_loop.mp3' build/apk-files.txt
echo "APK_SIZE_BYTES=$(stat -c%s build/CircleFootball-Android81-v2.7.apk)" | tee build/size.txt
