...
# Exact v2.2 gameplay stack + visuals only.
python3 v17_run.py
python3 v18_run.py
...
grep -q 'String fieldNameV33()' src/com/godnit/circlefootballlite/MainActivity.java
grep -q 'versionName="3.3.0"' AndroidManifest.xml
...
"$BT/apksigner" sign --ks build/app.keystore --ks-pass pass:android --key-pass pass:android --out build/CircleFootball-Android81-v3.3.apk build/aligned.apk
"$BT/apksigner" verify --verbose build/CircleFootball-Android81-v3.3.apk
"$BT/aapt" dump badging build/CircleFootball-Android81-v3.3.apk | tee build/badging.txt
grep -q "package: name='com.godnit.circlefootballlite.v33'" build/badging.txt
grep -q "versionCode='23'" build/badging.txt
grep -q "versionName='3.3.0'" build/badging.txt
grep -q "sdkVersion:'21'" build/badging.txt
unzip -l build/CircleFootball-Android81-v3.3.apk | tee build/apk-files.txt
echo "APK_SIZE_BYTES=$(stat -c%s build/CircleFootball-Android81-v3.3.apk)" | tee build/size.txt
